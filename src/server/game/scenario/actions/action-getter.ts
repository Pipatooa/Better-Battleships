...
import { Action, IActionSource } from './action';
 ...
 */
export async function getActions<T extends { [event in U]: IActionSource[] }, U extends string>(parsingContext: ParsingContext, eventNames: readonly U[], foreignEvents: readonly Partial<U>[], actionData: T): Promise<{ [event in U]: Action[] }> {
    ...
    for (const entry of Object.entries(actionData)) {
        const [event, actionSources] = entry as [U, IActionSource[]];
